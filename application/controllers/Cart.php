...
//     $token = bin2hex(random_bytes(8)); // 16 char token (8 bytes = 16 hex chars)
    
//     // HTTPS-compatible cookie setting
//     $cookie = array(
//         'name'     => 'guest_token',
//         'value'    => $token,
...
// }
    ...
    public function cart(){
        // echo "here"; exit;
        $cookie= $this->input->cookie('guest_token');
        // print_r($cookie);
    ...
    $full_weight = $product_weight . $product_kg_g;
        ...
        if (strtolower($product_kg_g) === 'kg') {
        $weightInGrams = $product_weight * 1000;
    } elseif (strtolower($product_kg_g) === 'g') {
...
public function loadcart(){
    $token= $this->input->cookie('guest_token');
    $ordertype = $this->session->userdata('order-type');
...
//    $shippingcharge = isset($shippingcharge) ? $shippingcharge : 0;
    ...
    echo json_encode(['success' => 'success', 'data' => $cartitems, 'message'=>'product added to cart successfully','totalprice'=>$sumofprice,'total_amount'=>$total_amout,'weightcalculation'=>$this->Productmodel->getweightcalc($token),'shippingcharge'=>$shipping , 'ordertype'=>$ordertype]);
...
if (count($cartitems) > 0) {
    foreach ($cartitems as $product) {
        ...
        $products= $this->Cartmodel->getcartitemprice($product_id);
...
$html .= '<li class="total bg-primary" style="background-color: #4a4848 !important;">';
    ...
    $html .= '<a href="' . base_url() . 'cart/cartview" class="butn small btn-cart white"><span>View Cart</span></a>';
    $html .= '</li>';
}
...
// 'value' => $token,
// 'expire' => 3600 * 24, // 1 day
// 'path' => '/woodsberg/', // Your subdirectory
// 'secure' => TRUE, // Required for HTTPS
// 'httponly' => TRUE // Security enhancement
// );

// $this->input->set_cookie($cookie);
// // setcookie('guest_token', $token, time() + (3600 * 24), '/', '', true, true);

// // DEBUG: Log cookie setting (remove after testing)
...
// echo json_encode([
// 'success' => 'success',
// 'message' => 'Shipping charge already set to ' . $shipping_charge,
// 'state' => $state
// ]);
// }
} else {
