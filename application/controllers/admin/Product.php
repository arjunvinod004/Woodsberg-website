        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            $data = array(
               ...
               'subcategory_id' => !empty($this->input->post('subcategory_id')) ? $this->input->post('subcategory_id'):0,
                ...
                'export_price' => $this->input->post('product_export_price'),
                ...
                'is_home'=>$this->input->post('is_home_add_hidden'),
                'is_bestseller'=>$this->input->post('is_bestseller_add_hidden'),
                'is_seasonaloffer'=>$this->input->post('is_seasonaloffer_add_hidden'),
                'out_of_stock'=>$this->input->post('out_of_stock_hidden'),
    ...
    public function searchProductOnKeyUp(){
        ...
        if (!empty($searchproducts)) {
            ...
            foreach ($searchproducts as $product) {
                ...
                $html .= '<h3 class="product-list__item-name">' . $product_name = ($product->product_name != '') ? $product->product_name : $product->product_name . '</h3>';
                $html .= '<p class="product-list__item-price">' . $product->retail_price . '</p>';
                ...
                $html .= '<div class="product-list__item-buttons-block-one">';
                            ...
                            $html .= '<a href="" class="product-list__item-buttons-block-btn btn6 nextavialable-modal  edit_product"
                            data-bs-toggle="modal" data-id="' . $product->product_id . '"
                            data-bs-target="#Edit-Product"><img class="product-list__item-button-img"
                                src="'.base_url().'assets/admin/images/next-available-time-icon.svg"
                                alt="add stock" width="23" height="24">Edit Product</a>' ;
    
                $html .= '<a data-bs-toggle="modal" data-bs-target="#delete-product"
                        data-id="' . $product->product_id . '"
                        href=""
                        class="product-list__item-buttons-block-btn btn6 edit-btn remove_product"><img
                            class="product-list__item-button-img"
                            src="'.base_url().'assets/admin/images/edit-dish-icon.svg" alt="add stock"
                            width="23" height="22">Remove Product</a>';
                $html .= '</div>';
                $html .= '</div>';
                $html .= '</div>';
                }
